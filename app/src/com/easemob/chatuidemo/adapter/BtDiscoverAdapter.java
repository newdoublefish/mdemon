...
public class BtDiscoverAdapter extends ArrayAdapter<DUser>  implements SectionIndexer
{
    ...
    public BtDiscoverAdapter(Context context, int resource, List<DUser> objects,Sidebar sidebar) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        if (position == 0) {//搜索框
            ...
            DUser user = getItem(position);
            if(user == null)
                Log.d("ContactAdapter", position + "");
            //设置nick，demo里不涉及到完整user，用username代替nick显示
            String username = user.getName();
            String header = user.getPasswd();
            /*
    ...
    @Override
    public DUser getItem(int position) {
